    ...
    return (
        <div className="space-y-6 animate-in fade-in h-full flex flex-col">
            <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm grid grid-cols-1 md:grid-cols-5 gap-4 items-end shrink-0">
                ...
                <div><label className="block text-xs font-bold text-slate-500 uppercase mb-1">Account Type</label><select className="w-full bg-slate-50 border border-slate-300 rounded-lg p-2 text-sm" value={accountType} onChange={e => { setAccountType(e.target.value); setSelectedAccountId(''); }}><option value="ALL">All Types</option><optgroup label="Business Partners"><option value="CUSTOMER">Customers</option><option value="SUPPLIER">Suppliers</option><option value="SUB SUPPLIER">Sub Suppliers</option><option value="VENDOR">Vendors</option></optgroup><optgroup label="Chart of Accounts"><option value="ASSET">Assets</option><option value="LIABILITY">Liabilities</option><option value="EXPENSE">Expenses</option><option value="REVENUE">Revenue</option></optgroup></select></div>
