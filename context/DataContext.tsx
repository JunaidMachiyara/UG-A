    ...
    */
    ...
    const addEmployee = async (employee: Employee) => {
        ...
        try {
            ...
            // Automatically create Employee Advance account
            const accountName = `Employee Advance - ${employee.name}`;
            
            // Check if account already exists (by name pattern)
            const existingAccount = state.accounts.find(a => 
                a.name === accountName && 
                (!a.factoryId || a.factoryId === currentFactory?.id)
            );
            
            if (existingAccount) {
                console.log(`ℹ️ Account already exists for ${employee.name}: ${existingAccount.code} - ${existingAccount.name}`);
                return;
            }
            
            // Find parent account "1130 - Employee Advances"
            const parentAccount = state.accounts.find(a => 
                (a.code === '1130' || a.name.includes('Employee Advances')) &&
                a.type === AccountType.ASSET &&
                (!a.factoryId || a.factoryId === currentFactory?.id)
            );
            
            // Find next available account code (1131-1830, skipping used codes)
            const usedCodes = new Set(state.accounts
                .filter(a => !a.factoryId || a.factoryId === currentFactory?.id)
                .map(a => parseInt(a.code))
                .filter(n => !isNaN(n) && n >= 1131 && n <= 1830)
            );
            
            // Codes to skip (already used in system)
            const skipCodes = new Set([
                ...Array.from({ length: 21 }, (_, i) => 1200 + i), // 1200-1220
                ...Array.from({ length: 21 }, (_, i) => 1300 + i), // 1300-1320
                1500, 1510, 1520, 1530, 1540, 1550, 1560, // Fixed Assets
                1710, 1720, 1730, 1740, // Depreciation
            ]);
            
            // Find next available code
            let nextCode = '';
            for (let code = 1131; code <= 1830; code++) {
                if (!usedCodes.has(code) && !skipCodes.has(code)) {
            ...
            if (!nextCode) {
                console.warn('⚠️ No available account code found in range 1131-1830. Please create account manually.');
                return;
            }
            
            // Create the employee advance account
            ...
            // Create the account
            await addAccount(employeeAccount);
            console.log(`✅ Employee Advance account created: ${nextCode} - ${accountName}`);
